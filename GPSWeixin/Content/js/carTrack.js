...
$(function () {
  ...
  $('#overlay').click(function () {
    ...
    //验证时间
    var validate = validateTime(1);
    if (validate) {
      //后台获取的GPS数据
      var dataObj = getBDPoints(vid, startTime, endTime);
...
//获取转换为百度坐标的点
function getBDPoints(vid, startTime, endTime) {
  var dataVar = {
    content: {  //功能数据字段
      vid: vid, //车辆编号
  ...
  $.ajax({
    async: false,
    url: hostUrl + "/actions/getPlayTraceData.aspx",
    type: "POST",
    dataType: "json",
...
function drawPoints(data, points) {
  //清空points,directions数组
  points.length = 0;
  directions.length = 0;
  for (var i = 0, len = data.length; i < len; i++) {
    points.push(new BMap.Point(data[i].lon, data[i].lat));
    directions.push(parseInt(data[i].direction));
  }
  ...
  //显示起点,终点marker
  var firstP = new BMap.Marker(points[0], { icon: createIcon(directions[0]) });
  var endP = new BMap.Marker(points[points.length - 1], { icon: createIcon(directions[directions.length - 1]) });
...
function trackPlay() {
  ...
  if (index > 0) {
    ...
    var stopP = new BMap.Marker(points[index], { icon: iconImg });
    map.addOverlay(stopP);
  }
...
//校验时间,timeInterval时间间隔:起止时间差,单位天
function validateTime(timeInterval) {
  var startTime = $('#beginTime').val();
  var endTime = $('#endTime').val();
  var nowTime = (new Date()).Format("yyyy-MM-dd hh:mm:ss");
  //timeInterval天的毫秒数
  var threeDays = 1000 * 3600 * 24 * timeInterval;
  ...
  if (difference < 0 || nowTime < endTime) {
    ...
    alert("起止时间必须在"+timeInterval+"天之内,且开始时间在三个月以内!");
    return false;
  } else {
    return true;
  }
